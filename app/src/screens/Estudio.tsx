...
export default function Estudio() {
  ...
  async function run() {
    ...
    setBusy(mode === "analyze" ? t("estudio.analyzing") : t("estudio.generating"));
    setErr(""); setOut("");
    try {
      const sys = `Voce e estrategista de conteudo viral pra Instagram e assessor de campanha politica. Use ESTE conhecimento atualizado do algoritmo:\n${algo}\n\nRespeite SEMPRE estas regras eleitorais:\n${TSE_2026}\nSe o conteudo pedir voto e for antes de 16/08/2026, avise. Se usar IA na peca final, lembre de rotular. PT-BR, direto, sem encher linguica.`;
      let user = "";
      if (mode === "analyze") {
    ...
    <div className="space-y-5 max-w-3xl">
      <div className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-panel)] p-5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <button onClick={run} disabled={!!busy} className="rounded-xl bg-[linear-gradient(135deg,#00e5c9,#0aa892)] px-5 py-2.5 font-bold text-[#04120f] hover:brightness-110 active:scale-[.99] disabled:opacity-50">{busy || (mode === "analyze" ? t("estudio.analyze") : t("estudio.generate"))}</button>
