...
async fn spike_following_count(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let resp = cli
        ...
        .header("cookie", format!("ds_user_id={ds}; csrftoken={csrf}"))
    ...
    if !status.is_success() {
        return Err(format!(
            "HTTP {status} (autenticacao falhou?) — corpo: {}",
    ...
    Ok(format!(
        "OK — autenticado com a sessao! uid={ds}, csrf={}..., HTTP {status}, 'users' no payload: {n}. MOTOR B VIAVEL.",
        &csrf[..csrf.len().min(6)]
    ))
}
